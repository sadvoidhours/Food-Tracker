 ...
 */
function parseNutritionData(serving) {
  return {
...
/**
 * Get accurate nutrition data for a food item
 */
export async function getNutritionData(foodName, brandName = null) {
  try {
...
export async function getNutritionForIngredients(ingredients) {
  try {
    const results = await Promise.all(
      ingredients.map(ingredient => getNutritionData(ingredient))
    );
