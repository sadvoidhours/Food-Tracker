 ...
 */
export async function analyzeFood(imageFile, dishName = '', allergyInfo = []) {
  if (!API_KEY) {
  ...
  try {
    ...
    try {
      ...
      // Add other sources from Gemini
      if (parsedResult.nutritionSources && parsedResult.nutritionSources.length > 0) {
        nutritionSources.push(...parsedResult.nutritionSources);
      }
 ...
 */
export async function analyzeIngredients(ingredientsList, dishName = '', allergyInfo = []) {
  if (!API_KEY) {
    throw new Error('Gemini API key is not configured. Please add your API key to .env file')
  }
